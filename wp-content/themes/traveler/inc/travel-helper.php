<?php
    ...
    if (!class_exists('TravelHelper')) {
        ...
        class TravelHelper
        {
            ...
            static function convertDateFormatColibri($date)
            {
        ...
        */
            static function transferDestination()
            {
                ...
                if (!empty($terms)) {
                    foreach ($terms as $term) {
                        $term_data = ST_Flight_Location_Models::inst()->get_data($term->term_id);
                        if ($term_data) {
                            $return[] = [
                                'id' => $term->term_id,
                                'name' => $term->name,
                                'address' => $term_data->map_address,
                                'type' => 'airport'
                            ];
                        }
                    }
            ...
            static function get_current_available_calendar($post_id)
            {
                if ($post_id != '') {
                    ...
                    $current_year = date('Y');
                    $start_default = $current_month . '/' . '01' . '/' . $current_year;
                    $date_return = date('Y-m-d');
                    for ($i = 0; $i <= 10; $i++) {
                        $start_date = date("m/d/Y", strtotime("+{$i} month", strtotime($start_default)));
                        $end_date = date("m/t/Y", strtotime($start_date));
                        if($post_type == 'st_activity'){
                            $data_available = AvailabilityHelper::_get_current_availability_activity($post_id, strtotime($start_date), strtotime($end_date));
                        }else{
                            $data_available = AvailabilityHelper::_get_current_availability_tour($post_id, strtotime($start_date), strtotime($end_date));
                        }
                        if (!empty($data_available)) {
                            foreach ($data_available as $k => $v) {
