    ...
    if ( !class_exists( 'STTour' ) ) {
        class STTour extends TravelerObject
        {
            ...
            public function st_filter_tour_ajax()
            {
                ...
                ob_clean();
                ob_end_flush();
                $count = balanceTags($tours->get_result_string());
                $result = array(
            ...
            function do_add_to_cart()
            {
                ...
                if ( $max_number > 0 ) {
                    ...
                    if ( $free_people < ( $adult_number + $child_number + $infant_number ) ) {
                        STTemplate::set_message( sprintf( __( 'This tour only vacant %d people at %s', ST_TEXTDOMAIN ), $free_people, $starttime ), 'danger' );
                        $pass_validate = FALSE;
                ...
                $data[ 'ori_price' ] = $sale_price + $extra_price + $package_hotel_price + $package_activity_price + $package_car_price;
                ...
                if ( $pass_validate ) {
                    ...
                    if ( $pass_validate ) {
                        //Add price Tour Package
                        STCart::add_cart( $item_id, $number, $sale_price + $extra_price + $package_hotel_price + $package_activity_price + $package_car_price, $data );
                    }
                }
